...
def sample_posterior_full(X_new, X, f_sample, ls,
    ...
    # Fix positive semi-definite warning
    # using https://stackoverflow.com/questions/41515522/numpy-positive-semi-definite-warning
    min_eig = np.min(np.real(np.linalg.eigvals(cond_cov)))
    if min_eig < 0:
        cond_cov -= 10*min_eig * np.eye(*cond_cov.shape)
