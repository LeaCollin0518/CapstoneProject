...
ensemble_weights_val = np.zeros(shape=(num_mcmc_steps, X_valid.shape[0], 3)) # 3 is the number of models 
...
for fold_id, (_, pred_index) in enumerate(kf.split(X_valid)):
    ...
    ensemble_weights_val[:, pred_index, :] = ensemble_weights_fold
...
with open(os.path.join(_SAVE_ADDR_PREFIX,
    ...
    # save the new files to new pkl files

with open(os.path.join(_SAVE_ADDR_PREFIX,
                       '{}/ensemble_weights_val.pkl'.format(family_name)), 'wb') as file:
    pk.dump(ensemble_weights_val, file, protocol=pk.HIGHEST_PROTOCOL)
# with open(os.path.join(_SAVE_ADDR_PREFIX,
#                        '{}/cond_weights_dict_val.pkl'.format(family_name)), 'wb') as file:
